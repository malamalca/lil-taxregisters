...
class LilTaxRegistersEvents implements EventListenerInterface
{
     ...
     * @return object
     */
    public function modifySidebar($event, $data)
    {
        $view = $event->getSubject();
        $sidebar = $data->sidebar;

        $sidebar['documents']['active'] = $sidebar['documents']['active'] ||
            ($view->request->getParam('controller') == 'BusinessPremises');
        ...
        $sidebar['documents']['items']['lookups']['submenu']['tax_registers'] = [
            ...
            'active' => $view->request->getParam('controller') == 'BusinessPremises',
        ];
    ...
    /**
     * Add tax block to invoices html
     *
     * @param object $event Event object.
    ...
    public function checkPKPassword($event)
    {
        ...
        if (($controller->getName() == 'Invoices') && ($controller->request->getParam('action') == 'edit')) {
    ...
    {
        $view = $event->getSubject();
        $currentUser = $view->get('currentUser');
    ...
    public function updateUser($event, $data, $options)
    {
        ...
        if (is_a($model, 'Lil\Model\Table\UsersTable')) {
            if (isset($data['cert_p12'])) {
                if (!empty($data['cert_p12']['tmp_name']) && file_exists($data['cert_p12']['tmp_name'])) {
                    $session = new Session();
